...
_uninstall() {
  ...
  echo "Script and dpkg hooks removed, please manually remove /etc/apt/sources.list.d/proxmox.sources and /etc/apt/ceph.sources if desired"
}
...
_install() {
  ...
  # create the proxmox-no-subscription.sources file
	echo "Creating /etc/apt/sources.list.d/proxmox-no-subscription.sources ..."
	cat <<- EOF > "/etc/apt/sources.list.d/proxmox-no-subscription.sources"
		Types: deb
  ...
  # create the ceph-no-subscription file
	echo "Creating /etc/apt/sources.list.d/ceph-no-subscription.sources ..."
...
emit_script() {
  base64 -d << 'YEET' | unxz
/Td6WFoAAATm1rRGAgAhARYAAAB0L+Wj4AbXA6hdABGIQkY99BY0cwoNj8U0dcgowbs41qLC+aejmGQY
...
2CEVk7vrz5O4N6e4erl0B2a6XTQ2u/ICDkCLaA2q4FIbMtlCsNHjkKPV5xQol7NPiDVGlu7emVJU/oVY
s5buueyZAACGL8v8OOswTgABxAfYDQAASlY0ELHEZ/sCAAAAAARZWg==
YEET
}
